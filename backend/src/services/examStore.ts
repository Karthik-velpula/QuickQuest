...
async function fetchExamRows(code: string): Promise<Exam | undefined> {
  ...
  const [questions] = await pool.query<mysql.RowDataPacket[] & QuestionRow[]>(
    `SELECT id, exam_code, question_order, document_question_number, question_text, option_a, option_b, option_c, option_d, correct_answer
     FROM exam_questions
     WHERE exam_code = ?
    ...
    for (const [index, question] of questions.entries()) {
      ...
      await connection.query(
        `INSERT INTO exam_questions
         (id, exam_code, question_order, document_question_number, question_text, option_a, option_b, option_c, option_d, correct_answer)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          `q${index + 1}-${code}`,
          code,
...
export async function getPublicExam(code: string): Promise<{ code: string; title: string; questions: PublicQuestion[] } | undefined> {
  ...
  return {
    code: exam.code,
    title: exam.title,
    questions: exam.questions.map(({ id, questionNumber, question, options }) => ({ id, questionNumber, question, options })),
  };
}
